          ...
          <div className="text-left overflow-hidden">
            <Breadcrumb
              ...
              items={[
                ...
                {
                  title: (
                    <span className="cursor-pointer hover:text-primary-500 transition-colors" onClick={() => navigate(ROUTES.ABOUT)}>
                      About
                    </span>
                  ),
                },
            ...
            />
            <h1 className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight text-left truncate">{isEditMode ? "Edit About Section" : "New About Section"}</h1>
          </div>
        </div>
        <div className="flex items-center gap-2 sm:gap-3">
          <Button variant="ghost" onClick={() => navigate(ROUTES.ABOUT)} className="h-10 sm:h-12 px-4 sm:px-6 rounded-xl sm:rounded-2xl border-2 font-bold text-slate-600 dark:text-slate-400 flex-1 sm:flex-none">
            Cancel
          </Button>
          ...
          <Card className="min-w-0 rounded-2xl sm:rounded-[32px] shadow-xl border-0 overflow-hidden bg-white dark:bg-slate-900 text-left p-4 sm:p-8 self-start">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 text-left">
              ...
              <Form.Item label={<span className="font-bold text-slate-600 dark:text-slate-400 text-left">Priority</span>} name="priority" className="col-span-2 md:col-span-1 text-left">
                <InputNumber<number> min={0} controls={false} parser={(value) => Number((value || "").toString().replace(/[^\d]/g, ""))} className="w-full h-12 rounded-xl focus:ring-primary-500 text-left dark:bg-slate-800 dark:text-white dark:border-slate-700 [&_.ant-input-number-input]:!h-12 [&_.ant-input-number-input]:!leading-[48px]" prefix={<Hash size={14} className="text-slate-400" />} />
              </Form.Item>
                ...
                {imageUrl ? (
                  <div className="space-y-4 sm:space-y-6 text-left">
                    ...
                    <div className="p-3 sm:p-4 bg-gray-50 dark:bg-white/5 rounded-xl sm:rounded-2xl border border-gray-100 dark:border-white/10 text-left">
                      <p className="text-[10px] sm:text-xs font-bold text-slate-500 dark:text-white/50 line-clamp-3 text-left">{descriptionValue || "Description will appear here for a quick preview."}</p>
                    </div>
                  </div>
                ) : (
