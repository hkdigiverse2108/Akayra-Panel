...
const IgPostManagement: React.FC = () => {
  ...
  const { items: igPosts, loading, total, currentPage, pageSize, searchTerm, activeFilter, setSearchTerm, setCurrentPage, setPageSize, setActiveFilter, toggleSort, getSortIcon } = useManagementData({
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 text-left">
        <div className="text-left px-1">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium mt-1 text-sm sm:text-base">
            Curate Instagram-style posts, links, and priorities.
          </p>
        </div>
                ...
                return (
                  <div key={igPostId || index} className="group relative bg-gray-50/50 dark:bg-slate-800/30 rounded-3xl border border-gray-100 dark:border-slate-800 overflow-hidden hover:border-primary-500/30 transition-all flex flex-col p-4 shadow-sm">
                    ...
                    <div className="flex items-start justify-between gap-3 text-left">
                      ...
                      <div className={cn("px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest", isActive(igPost) ? "text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10" : "text-slate-400 bg-slate-100 dark:bg-slate-800")}
                      >
                        {isActive(igPost) ? 'Active' : 'Inactive'}
                      </div>
                    </div>
