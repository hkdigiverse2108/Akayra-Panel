  ...
  return (
    <div className="space-y-4 sm:space-y-8 animate-in fade-in duration-500">
      ...
      <Card className="!p-0 overflow-hidden rounded-2xl sm:rounded-3xl shadow-xl border-0 bg-white dark:bg-slate-900">
        <TableToolbar searchTerm={searchTerm} onSearchChange={setSearchTerm} placeholder="Search brands..." activeFilter={activeFilter} onActiveFilterChange={setActiveFilter} showViewToggle viewType={viewType} onViewTypeChange={setViewType} />
                        ...
                        <div className="flex items-center gap-3 sm:gap-4 text-left">
                          <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl bg-slate-100 dark:bg-slate-800 overflow-hidden flex items-center justify-center border border-gray-100 dark:border-slate-700 shadow-sm transition-transform group-hover:scale-110 shrink-0">
                            {brand.image && !brokenImages[brand._id] ? (
                              <button type="button" onClick={() => openImageModal(brand.image, brand.name || brand.title)} className="relative h-full w-full cursor-pointer group/thumb" title="View image">
                                <Image src={brand.image} alt={brand.name || brand.title} className="h-full w-full object-contain p-1" preview={false} onError={() => handleImageError(brand._id)} />
                      ...
                      <td className="px-4 sm:px-8 py-5 text-right">
                        <div className="flex items-center justify-end gap-1.5 sm:gap-2">
                          <Tooltip title={brand.isActive ? "Deactivate" : "Activate"}>
                            <button onClick={() => handleToggleStatus(brand)} disabled={isActionLoading} className={cn("p-2 rounded-xl transition-all shadow-sm", brand.isActive ? "text-emerald-500 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-500/10 dark:hover:bg-emerald-500/20" : "text-slate-300 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700")}>
                              {brand.isActive ? <ToggleRight size={20} className="sm:w-6 sm:h-6" /> : <ToggleLeft size={20} className="sm:w-6 sm:h-6" />}
                            </button>
                          </Tooltip>
                          <Tooltip title="Edit">
                            <button onClick={() => navigate(`${ROUTES.BRANDS}/edit/${brand._id}`)} className="p-2 bg-primary-50 hover:bg-primary-100 dark:bg-primary-500/10 dark:hover:bg-primary-500/20 text-primary-600 dark:text-primary-400 rounded-xl transition-all shadow-sm">
                              <Edit size={18} className="sm:w-5 sm:h-5" />
                            </button>
                          </Tooltip>
                          <Tooltip title="Delete">
                            <button onClick={() => handleDeleteClick(brand._id)} className="p-2 bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 text-red-600 dark:text-red-400 rounded-xl transition-all shadow-sm">
              ...
              brands.map((brand: any) => (
                <div key={brand._id} className="group relative bg-gray-50/50 dark:bg-slate-800/30 rounded-3xl border border-gray-100 dark:border-slate-800 overflow-hidden hover:border-primary-500/30 transition-all flex flex-col items-center p-6 text-center shadow-sm">
                  <div className="h-20 w-20 bg-white dark:bg-slate-900 rounded-2xl flex items-center justify-center shadow-sm mb-4 transition-transform group-hover:scale-110 border border-gray-50 dark:border-slate-700">
                    {brand.image && !brokenImages[brand._id] ? (
                      <button type="button" onClick={() => openImageModal(brand.image, brand.name || brand.title)} className="relative h-full w-full cursor-pointer group/thumb" title="View image">
                        <Image src={brand.image} alt={brand.name || brand.title} preview={false} className="h-full w-full object-contain p-2" onError={() => handleImageError(brand._id)} />
                        <div className="absolute inset-0 bg-black/0 group-hover/thumb:bg-black/30 transition-colors" />
                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/thumb:opacity-100 transition-opacity">
        ...
        <TableFooter currentPage={currentPage} pageSize={pageSize} total={total} onPageChange={setCurrentPage} onPageSizeChange={setPageSize} resourceName="brands" />
      </Card>

      <ConfirmModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={confirmDelete} loading={isActionLoading} message="Are you sure you want to delete this brand?" />
