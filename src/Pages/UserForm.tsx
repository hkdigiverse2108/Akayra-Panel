    ...
    <div className="space-y-6 animate-in fade-in duration-500 text-left">
        ...
        <div className="text-left">
          <Breadcrumb
            ...
            items={[
              ...
              {
                title: (
                  <span className="cursor-pointer hover:text-primary-500 transition-colors" onClick={() => navigate(ROUTES.USERS)}>
                    Users
                  </span>
                ),
              },
          ...
          />
          <h1 className="text-3xl font-black text-slate-900 dark:text-white tracking-tight text-left">{isEditMode ? "Edit User Profile" : "Create New User"}</h1>
        </div>
        <Button type="default" icon={<ArrowLeft size={16} />} onClick={() => navigate(ROUTES.USERS)} className="h-12 px-6 rounded-2xl font-bold border-2 flex items-center gap-2">
          Back to List
        </Button>
      </div>
      ...
      <Card className="rounded-3xl shadow-xl border-0 overflow-hidden bg-white dark:bg-slate-900 text-left">
        <div className="p-1 text-left">
          <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ role: "user", isActive: true }} requiredMark={false} className="max-w-4xl mx-auto py-8 text-left">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 text-left">
              ...
              <Form.Item label={<span className="font-bold text-slate-600 dark:text-slate-400 text-left">Password</span>} name="password" rules={[{ required: !isEditMode, message: "Password is required" }]} extra={<span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1 block text-left">{isEditMode ? "Leave blank to keep existing password" : "Create a secure password"}</span>} className="text-left">
            ...
            <div className="mt-12 flex items-center gap-4 text-left">
              <Button type="primary" htmlType="submit" loading={addUser.isPending || editUser.isPending || fetching} icon={<Save size={18} />} className="h-14 px-10 rounded-2xl font-black text-lg bg-primary-600 hover:bg-primary-700 border-0 flex items-center gap-2 shadow-lg shadow-primary-500/20">
                {isEditMode ? "Update User" : "Create User"}
              </Button>
              <Button onClick={() => navigate(ROUTES.USERS)} className="h-14 px-8 rounded-2xl font-bold text-slate-600 border-2">
                Cancel
              </Button>
            </div>
          </Form>
