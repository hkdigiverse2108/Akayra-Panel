...
import { Plus, Edit, Trash2, ToggleLeft, ToggleRight, Layers, ImageIcon, X } from "lucide-react";
...
const CategoryManagement: React.FC = () => {
  ...
  const openImageModal = (imageUrl: string, title?: string) => {
    ...
    setActiveTitle(title || "Category Image");
    ...
    <div className="space-y-4 sm:space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 text-left">
        <div className="text-left px-1">
          <h1 className="text-2xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight">Category Catalog</h1>
          <p className="text-slate-500 dark:text-slate-400 font-medium mt-1 text-sm sm:text-base">Organize your products into nested hierarchies.</p>
        </div>
        <Button onClick={() => navigate(`${ROUTES.CATEGORIES}/add`)} className="h-10 sm:h-12 px-6 rounded-2xl flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-700 text-white font-black shadow-lg shadow-primary-500/20 w-full md:w-auto">
          <Plus size={20} /> Add Category
        </Button>
      </div>

      <Card className="!p-0 overflow-hidden rounded-2xl sm:rounded-3xl shadow-xl border-0 bg-white dark:bg-slate-900">
        <TableToolbar searchTerm={searchTerm} onSearchChange={setSearchTerm} placeholder="Search categories..." activeFilter={activeFilter} onActiveFilterChange={setActiveFilter} showViewToggle viewType={viewType} onViewTypeChange={setViewType} />
            ...
            <table className="w-full text-left border-collapse min-w-[500px] md:min-w-full">
              ...
              <tbody className="divide-y divide-gray-100 dark:divide-slate-800 text-left">
                {loading && categories.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="px-8 py-20 text-center text-slate-400 font-bold italic tracking-wider animate-pulse">
                      Loading categories...
                    </td>
                  </tr>
                ) : categories.length === 0 ? (
                  <tr>
                    <td colSpan={3} className="px-8 py-20 text-center text-slate-400 font-bold italic tracking-wider">
                      No categories found.
                    </td>
                  </tr>
                ) : (
                      ...
                      <td className="px-4 sm:px-8 py-5">
                        <div className="flex items-center gap-3 sm:gap-4 text-left">
                          <div className="h-10 w-10 sm:h-12 sm:w-12 rounded-xl bg-slate-100 dark:bg-slate-800 overflow-hidden flex items-center justify-center border border-gray-100 dark:border-slate-700 shadow-sm transition-transform group-hover:scale-110 shrink-0">
                            {category.image && !brokenImages[category._id] ? (
                              <button type="button" onClick={() => openImageModal(category.image, category.name)} className="relative h-full w-full cursor-pointer group/thumb" title="View image">
                                <Image src={category.image} alt={category.name} className="h-full w-full object-contain p-1" preview={false} onError={() => handleImageError(category._id)} />
                          ...
                          <p className="text-sm font-black text-slate-900 dark:text-white leading-none capitalize truncate max-w-[150px] sm:max-w-none">{category.name}</p>
                        </div>
                      </td>
                      <td className="px-4 sm:px-8 py-5 text-right">
                        <div className="flex items-center justify-end gap-1.5 sm:gap-2">
                          <Tooltip title={category.isActive ? "Deactivate" : "Activate"}>
                            <button onClick={() => handleToggleStatus(category)} disabled={isActionLoading} className={cn("p-2 rounded-xl transition-all shadow-sm", category.isActive ? "text-emerald-500 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-500/10 dark:hover:bg-emerald-500/20" : "text-slate-300 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700")}>
                              {category.isActive ? <ToggleRight size={20} className="sm:w-6 sm:h-6" /> : <ToggleLeft size={20} className="sm:w-6 sm:h-6" />}
                            </button>
                          </Tooltip>
                          <Tooltip title="Edit">
                            <button onClick={() => navigate(`${ROUTES.CATEGORIES}/edit/${category._id}`)} className="p-2 bg-primary-50 hover:bg-primary-100 dark:bg-primary-500/10 dark:hover:bg-primary-500/20 text-primary-600 dark:text-primary-400 rounded-xl transition-all shadow-sm">
                              <Edit size={18} className="sm:w-5 sm:h-5" />
                            </button>
                          </Tooltip>
                          <Tooltip title="Delete">
                            <button onClick={() => handleDeleteClick(category._id)} className="p-2 bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 text-red-600 dark:text-red-400 rounded-xl transition-all shadow-sm">
              ...
              categories.map((category: any) => (
                <div key={category._id} className="group relative bg-gray-50/50 dark:bg-slate-800/30 rounded-3xl border border-gray-100 dark:border-slate-800 overflow-hidden hover:border-primary-500/30 transition-all flex flex-col items-center p-6 text-center shadow-sm">
                  <div className="h-20 w-20 bg-white dark:bg-slate-900 rounded-2xl flex items-center justify-center shadow-sm mb-4 transition-transform group-hover:scale-110 border border-gray-50 dark:border-slate-700">
                    {category.image && !brokenImages[category._id] ? (
                      <button type="button" onClick={() => openImageModal(category.image, category.name)} className="relative h-full w-full cursor-pointer group/thumb" title="View image">
                        <Image src={category.image} alt={category.name} preview={false} className="h-full w-full object-contain p-2" onError={() => handleImageError(category._id)} />
                        <div className="absolute inset-0 bg-black/0 group-hover/thumb:bg-black/30 transition-colors" />
                        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover/thumb:opacity-100 transition-opacity">
        ...
        <TableFooter currentPage={currentPage} pageSize={pageSize} total={total} onPageChange={setCurrentPage} onPageSizeChange={setPageSize} resourceName="categories" />
      </Card>

      <ConfirmModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={confirmDelete} loading={isActionLoading} message="Are you sure you want to delete this category? This action cannot be undone." />
