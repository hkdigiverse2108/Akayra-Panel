...
const CouponForm: React.FC = () => {
  ...
  useEffect(() => {
    if (isEditMode && couponFromState) {
      const normalizedProductIds = Array.isArray(couponFromState?.productIds) ? couponFromState.productIds.map((p: any) => (typeof p === "string" ? p : p?._id)).filter(Boolean) : undefined;
      form.setFieldsValue({
        ...couponFromState,
          ...
          <div className="text-left overflow-hidden">
            <Breadcrumb
              ...
              items={[
                ...
                {
                  title: (
                    <span className="cursor-pointer hover:text-primary-500 transition-colors" onClick={() => navigate(ROUTES.COUPONS)}>
                      Coupons
                    </span>
                  ),
                },
            ...
            />
            <h1 className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight text-left truncate">{isEditMode ? "Edit Coupon" : "New Coupon"}</h1>
          </div>
        </div>
        <div className="flex items-center gap-2 sm:gap-3">
          <Button variant="ghost" onClick={() => navigate(ROUTES.COUPONS)} className="h-10 sm:h-12 px-4 sm:px-6 rounded-xl sm:rounded-2xl border-2 font-bold text-slate-600 dark:text-slate-400 flex-1 sm:flex-none">
            Cancel
          </Button>
