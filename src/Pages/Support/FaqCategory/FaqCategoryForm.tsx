    ...
    <div className="max-w-2xl mx-auto space-y-4 sm:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 px-4 sm:px-0 pb-12 sm:pb-0 text-left">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-left">
        <div className="flex items-center gap-3 sm:gap-4 text-left">
          <button onClick={() => navigate(ROUTES.FAQ_CATEGORIES)} className="p-2 sm:p-3 hover:bg-white dark:hover:bg-slate-800 rounded-xl sm:rounded-2xl transition-all shadow-sm hover:shadow text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 border border-gray-100 dark:border-slate-800">
          ...
          <div className="text-left overflow-hidden">
            <Breadcrumb
              ...
              items={[
                ...
                {
                  title: (
                    <span className="cursor-pointer hover:text-primary-500 transition-colors" onClick={() => navigate(ROUTES.FAQ_CATEGORIES)}>
                      FAQ Categories
                    </span>
                  ),
                },
            ...
            />
            <h1 className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight text-left truncate">{isEditMode ? "Edit Taxonomy" : "New Group"}</h1>
          </div>
        </div>
      </div>
      ...
      <Card className="rounded-[24px] sm:rounded-[32px] border-0 shadow-xl overflow-hidden p-6 sm:p-8 bg-white dark:bg-slate-900 text-left">
        <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ isActive: true }} requiredMark={false} className="text-left">
          <Form.Item name="title" label={<span className="text-[10px] sm:text-xs font-black text-slate-400 uppercase tracking-widest text-left">Category Name</span>} rules={[{ required: true, message: "Name is required" }]} className="text-left mb-4 sm:mb-6">
