  ...
  return (
    <div className="space-y-4 sm:space-y-8 animate-in fade-in duration-500">
      ...
      <Card className="!p-0 overflow-hidden rounded-2xl sm:rounded-3xl shadow-xl border-0 bg-white dark:bg-slate-900">
        <TableToolbar searchTerm={searchTerm} onSearchChange={setSearchTerm} placeholder="Search faq categories..." activeFilter={activeFilter} onActiveFilterChange={setActiveFilter} showViewToggle viewType={viewType} onViewTypeChange={setViewType} />
                    ...
                    <tr key={category._id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors group cursor-default">
                      ...
                      <td className="px-4 sm:px-8 py-5 text-right">
                        <div className="flex items-center justify-end gap-1.5 sm:gap-2">
                          ...
                          <Tooltip title="Edit">
                            <button onClick={() => navigate(`${ROUTES.FAQ_CATEGORIES}/edit/${category._id}`, { state: { category } })} className="p-2 bg-primary-50 hover:bg-primary-100 dark:bg-primary-500/10 dark:hover:bg-primary-500/20 text-primary-600 dark:text-primary-400 rounded-xl transition-all shadow-sm">
              ...
              categories.map((category: any) => (
                <div key={category._id} className="group relative bg-gray-50/50 dark:bg-slate-800/30 rounded-[32px] border border-gray-100 dark:border-slate-800 p-8 flex flex-col items-center text-center transition-all hover:border-primary-500/30 shadow-sm hover:shadow-md h-full">
                  ...
                  <div className="mt-auto flex items-center gap-2">
                    <button onClick={() => handleToggleStatus(category)} disabled={isActionLoading} className={cn("h-10 w-10 rounded-xl flex items-center justify-center transition-all shadow-sm", category.isActive ? "bg-emerald-50 text-emerald-500 shadow-emerald-500/10" : "bg-white dark:bg-slate-900 text-slate-300")}>
                      {category.isActive ? <ToggleRight size={24} /> : <ToggleLeft size={24} />}
                    </button>
                    <button onClick={() => navigate(`${ROUTES.FAQ_CATEGORIES}/edit/${category._id}`, { state: { category } })} className="h-10 w-10 bg-white dark:bg-slate-900 text-primary-600 rounded-xl flex items-center justify-center shadow-sm border border-gray-100 dark:border-slate-800 hover:bg-primary-600 hover:text-white transition-all">
                      <Edit size={16} />
                    </button>
                    <button onClick={() => handleDeleteClick(category._id)} className="h-10 w-10 bg-white dark:bg-slate-900 text-red-600 rounded-xl flex items-center justify-center shadow-sm border border-gray-100 dark:border-slate-800 hover:bg-red-600 hover:text-white transition-all">
                      <Trash2 size={16} />
                    </button>
                  </div>
                </div>
      ...
      <ConfirmModal isOpen={isDeleteModalOpen} onClose={() => setIsDeleteModalOpen(false)} onConfirm={confirmDelete} loading={isActionLoading} message="Are you sure you want to delete this category?" />
    </div>
  );
};
