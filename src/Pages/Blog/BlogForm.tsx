...
const BlogForm: React.FC = () => {
  ...
  useEffect(() => {
    if (isEditMode && blogResponse?.data) {
      ...
      form.setFieldsValue({
        ...data,
        categoryIds: Array.isArray(data?.categoryIds) ? data.categoryIds.map((item: any) => item?._id || item).filter(Boolean) : data?.categoryIds,
      });
    }
          ...
          <div className="text-left overflow-hidden">
            <Breadcrumb
              ...
              items={[
                ...
                {
                  title: (
                    <span className="cursor-pointer hover:text-primary-500 transition-colors" onClick={() => navigate(ROUTES.BLOGS)}>
                      Blogs
                    </span>
                  ),
                },
            ...
            />
            <h1 className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight text-left truncate">{isEditMode ? "Edit Article" : "New Article"}</h1>
          </div>
        </div>
        <div className="flex items-center gap-2 sm:gap-3">
          <Button variant="ghost" onClick={() => navigate(ROUTES.BLOGS)} className="h-10 sm:h-12 px-4 sm:px-6 rounded-xl sm:rounded-2xl border-2 font-bold text-slate-600 dark:text-slate-400 flex-1 sm:flex-none">
            Cancel
          </Button>
            ...
            <div className="space-y-6 text-left">
              <div className="aspect-square rounded-[40px] overflow-hidden bg-gray-50 dark:bg-white/5 border border-gray-100 dark:border-white/10 shadow-2xl relative group text-left">
                {imageUrl ? (
                  ...
                  <div className="h-full w-full flex items-center justify-center text-slate-300 dark:text-white/10 text-left">
                    <ImageIcon size={64} />
                  </div>
                )}
                  ...
                  {imageUrl ? (
                    <div className="flex items-center justify-between gap-3 px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
                      <div className="flex items-center gap-3 min-w-0">
                        <img src={imageUrl} alt="Blog Thumbnail" className="h-12 w-12 rounded-xl object-cover border border-slate-200 dark:border-slate-700" />
                        <span className="text-xs text-slate-600 dark:text-slate-300 truncate">{imageUrl}</span>
                      </div>
                      <button type="button" onClick={() => form.setFieldsValue({ thumbnail: "" })} className="h-8 w-8 rounded-lg flex items-center justify-center text-slate-400 hover:text-red-600">
                        <X size={14} />
                      </button>
                    </div>
                  ) : (
          ...
          <Card className="rounded-[40px] border-0 shadow-xl overflow-hidden p-8 bg-white dark:bg-slate-900 text-left">
            ...
            <div className="space-y-6 text-left">
              ...
              <Form.Item name="categoryIds" label={<span className="text-xs font-black text-slate-400 uppercase tracking-widest text-left">Categories</span>} className="text-left">
                <Select mode="multiple" placeholder="Select categories" className="custom-select h-auto [&_.ant-select-selector]:!rounded-2xl [&_.ant-select-selector]:!bg-gray-50/50 [&_.ant-select-selector]:!border-0 text-left" showSearch filterOption={(input, option) => ((option?.children as any) || "").toString().toLowerCase().includes(input.toLowerCase())}>
      ...
      <UploadImage
        ...
        onSelect={(items: UploadItem[]) => {
          ...
          if (first?.url || first?.path) {
            form.setFieldsValue({ thumbnail: first.url || first.path });
          }
        }}
      />
    </div>
