    ...
    <div className="space-y-4 sm:space-y-8 animate-in fade-in duration-500">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 text-left">
        <div className="text-left px-1">
          ...
          <p className="text-slate-500 dark:text-slate-400 font-medium mt-1 text-sm sm:text-base">
            Track customer messages and respond quickly.
          </p>
        </div>
      </div>
            ...
            <table className="w-full text-left border-collapse min-w-[700px] md:min-w-full">
              <thead>
                <tr className="bg-gray-50/50 dark:bg-slate-800/30">
                  ...
                  <th className="px-4 sm:px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-gray-100 dark:border-slate-800 hidden md:table-cell">
                    Phone
                  </th>
                  <th className="px-4 sm:px-8 py-5 text-[10px] font-black text-slate-400 uppercase tracking-widest border-b border-gray-100 dark:border-slate-800 hidden md:table-cell">
                    Subject
                ...
                {loading && inquiries.length === 0 ? (
                  ...
                  inquiries.map((inquiry: any, index: number) => (
                    <tr key={inquiry._id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors group cursor-default">
                      ...
                      <td className="px-4 sm:px-8 py-5">
                        <div className="flex items-center gap-3 sm:gap-4 text-left">
                          ...
                          <div className="text-left overflow-hidden">
                            <p className="text-sm font-black text-slate-900 dark:text-white leading-none capitalize truncate max-w-[150px] sm:max-w-xs">
                              {getName(inquiry)}
                            </p>
                      ...
                      <td className="px-4 sm:px-8 py-5 text-[10px] sm:text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-tighter truncate max-w-[140px] hidden md:table-cell">
                        {getPhone(inquiry)}
                      </td>
                      <td className="px-4 sm:px-8 py-5 text-[10px] sm:text-xs font-bold text-slate-600 dark:text-slate-400 uppercase tracking-tighter truncate max-w-[180px] hidden md:table-cell">
                        {getSubject(inquiry)}
                      </td>
                      <td className="px-4 sm:px-8 py-5">
                        <div className="flex items-start gap-2 text-xs font-medium text-slate-600 dark:text-slate-400 max-w-[260px] sm:max-w-[320px]">
                          ...
                          <div>
                            ...
                            <p className="mt-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest lg:hidden">
                              {getDate(inquiry)}
                            </p>
                          </div>
                        </div>
                      ...
                      <td className="px-4 sm:px-8 py-5 text-right">
                        <div className="flex items-center justify-end gap-1.5 sm:gap-2">
                          <Tooltip title="View">
                            <button onClick={() => handleViewClick(inquiry)} className="p-2 bg-slate-50 hover:bg-slate-100 dark:bg-slate-700/30 dark:hover:bg-slate-700/50 text-slate-700 dark:text-slate-200 rounded-xl transition-all shadow-sm"><ViewIcon size={18} className="sm:w-5 sm:h-5" /></button>
                          </Tooltip>
                          <Tooltip title="Delete">
                            <button onClick={() => handleDeleteClick(inquiry._id)} disabled={deleteMutation.isPending} className="p-2 bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 text-red-600 dark:text-red-400 rounded-xl transition-all shadow-sm"><Trash2 size={18} className="sm:w-5 sm:h-5" /></button>
                          </Tooltip>
                        </div>
                      </td>
                    </tr>
              ...
              inquiries.map((inquiry: any) => (
                <div key={inquiry._id} className="group relative bg-gray-50/50 dark:bg-slate-800/30 rounded-3xl border border-gray-100 dark:border-slate-800 overflow-hidden hover:border-primary-500/30 transition-all flex flex-col p-4 shadow-sm">
                  ...
                  <div className="mt-4 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                    <div className="flex items-center gap-2"><MessageSquare size={12} /> {getSubject(inquiry)}</div>
                    <div className="flex items-center gap-2 mt-2"><Clock size={12} /> {getDate(inquiry)}</div>
                  </div>
                  ...
                  <div className="mt-4 flex items-center gap-1.5">
                    <Tooltip title="View">
                      <button onClick={() => handleViewClick(inquiry)} className="p-2 bg-slate-50 hover:bg-slate-100 dark:bg-slate-700/30 dark:hover:bg-slate-700/50 text-slate-700 dark:text-slate-200 rounded-xl transition-all shadow-sm"><ViewIcon size={18} /></button>
                    </Tooltip>
                    <Tooltip title="Delete">
                      <button onClick={() => handleDeleteClick(inquiry._id)} disabled={deleteMutation.isPending} className="p-2 bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 text-red-600 dark:text-red-400 rounded-xl transition-all shadow-sm"><Trash2 size={18} /></button>
                    </Tooltip>
                  </div>
                </div>
              ))
      ...
      <Modal open={isMessageModalOpen} onCancel={() => setIsMessageModalOpen(false)} footer={null} title={null} centered destroyOnClose closable={false} width={720} rootClassName="message-modal">
        <div className="space-y-4">
          <div className="text-left"><h3 className="text-lg font-black text-slate-900 dark:text-white tracking-tight">Inquiry Details</h3><p className="text-xs font-semibold text-slate-400 dark:text-slate-500 mt-1">Contact inquiry from user</p></div>
          <div className="grid gap-3 text-sm text-slate-700 dark:text-slate-200">
            <div className="flex items-center gap-2"><User size={14} className="text-slate-400" /><span className="font-semibold">Name:</span><span>{getName(selectedInquiry)}</span></div>
            <div className="flex items-center gap-2"><Mail size={14} className="text-slate-400" /><span className="font-semibold">Email:</span><span className="break-all">{getEmail(selectedInquiry)}</span></div>
            <div className="flex items-center gap-2"><Phone size={14} className="text-slate-400" /><span className="font-semibold">Phone:</span><span>{getPhone(selectedInquiry)}</span></div>
            <div className="flex items-center gap-2"><Clock size={14} className="text-slate-400" /><span className="font-semibold">Date:</span><span>{getDate(selectedInquiry)}</span></div>
          </div>
          ...
          <div className="flex justify-end pt-2"><Button variant="secondary" onClick={() => setIsMessageModalOpen(false)} className="h-10 px-5 rounded-xl font-bold">Close</Button></div>
        </div>
      </Modal>
    </div>
  );
