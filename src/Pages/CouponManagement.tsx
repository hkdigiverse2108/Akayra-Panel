...
const CouponManagement: React.FC = () => {
  ...
  //data fetching
  const { items: coupons, loading, total, currentPage, pageSize, searchTerm, activeFilter, setSearchTerm, setCurrentPage, setPageSize, setActiveFilter, toggleSort, getSortIcon } = useManagementData({
                ...
                {loading && coupons.length === 0 ? (
                  ...
                  coupons.map((coupon: any, index: number) => {
                    ...
                    <tr key={couponId || index} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors group cursor-default">
                      ...
                      <td className="px-4 sm:px-8 py-5">
                        <div className="flex items-center gap-3 sm:gap-4 text-left">
                          ...
                          <div className="text-left overflow-hidden">
                            <p className="text-sm font-black text-slate-900 dark:text-white leading-snug pb-0.5 uppercase tracking-widest truncate max-w-[200px] sm:max-w-xs">
                              {getCode(coupon)}
                            </p>
                            <div className="flex items-center gap-2 text-[10px] font-bold text-slate-400 mt-1 truncate max-w-[220px] sm:max-w-xs">
                              ...
                              <span className={cn("inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest", badgeTone)}>
                      ...
                      <td className="px-4 sm:px-8 py-5">
                        <span className={cn("inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest", badgeTone)}>
                          {getDiscountValue(coupon)}
                        </span>
                      </td>
                      <td className="px-4 sm:px-8 py-5">
                        <span className={cn("inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest", badgeTone)}>
                          {coupon?.type === 'buyXgetY' ? (coupon?.buyQty ?? '-') : '-'}
                        </span>
                      </td>
                      <td className="px-4 sm:px-8 py-5">
                        <span className={cn("inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest", badgeTone)}>
                          {coupon?.type === 'buyXgetY' ? (coupon?.getFreeQty ?? '-') : '-'}
                        </span>
                      </td>
                      <td className="px-4 sm:px-8 py-5">
                        <span className={cn("inline-flex items-center px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest", badgeTone)}>
                  ...
                  <div key={couponId || index} className="group relative bg-gray-50/50 dark:bg-slate-800/30 rounded-3xl border border-gray-100 dark:border-slate-800 overflow-hidden hover:border-primary-500/30 transition-all flex flex-col p-4 shadow-sm">
                    <div className="flex items-start justify-between gap-3">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <h3 className="text-sm font-black text-slate-900 dark:text-white uppercase tracking-widest line-clamp-1">
                            {getCode(coupon)}
                          </h3>
                      ...
                      <div className={cn("px-2.5 py-1 rounded-full text-[10px] font-black uppercase tracking-widest", isActive(coupon) ? "text-emerald-600 bg-emerald-50 dark:bg-emerald-500/10" : "text-slate-400 bg-slate-100 dark:bg-slate-800")}>
                        {isActive(coupon) ? 'Active' : 'Inactive'}
                      </div>
                    </div>

                    <div className="mt-4 space-y-2 text-[10px] font-bold text-slate-400 uppercase tracking-widest">
                      <div className="flex items-center gap-2"><Calendar size={12} /> {getValidity(coupon)}</div>
                      <div className="flex items-center gap-2"><Users size={12} /> Usage {getUsage(coupon)}</div>
                    </div>
