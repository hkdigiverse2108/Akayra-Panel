...
const SettingsManagement: React.FC = () => {
  ...
  useEffect(() => {
    if (settings) {
      form.setFieldsValue({
        ...settings,
        securePaymentImages: Array.isArray(settings?.securePaymentImages) ? settings.securePaymentImages.join("\n") : settings?.securePaymentImages || "",
      });
    }
  }, [settings, form]);
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-4 sm:space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500 px-4 sm:px-0 pb-12 sm:pb-0 text-left">
      ...
      <Form form={form} preserve={true} layout="vertical" onFinish={onFinish} requiredMark={false} disabled={!isEditing} initialValues={{ isOutOfStockProductShow: true, freeDeliveryAbove: 0, isCODAvailable: true, isRazorpay: false, isPhonePe: false, isShipRocket: false, isCashFree: false }} className="text-left">
        <Card className="rounded-[15px] sm:rounded-[25px] shadow-xl border border-gray-100 dark:border-slate-800 p-4 sm:p-8 text-left">
          <div className="flex flex-col gap-6">
                  ...
                  return (
                    <>
                      <div className="flex items-center gap-3 mb-6 text-left">
                        ...
                        <div>
                          <h2 className="text-lg sm:text-xl font-black text-slate-900 dark:text-white tracking-tight">{activeTabMeta?.title}</h2>
                          <p className="text-xs text-slate-500 dark:text-slate-400 font-semibold mt-1">{activeTabMeta?.description}</p>
                        </div>
                      </div>

                      <div className={`space-y-5 ${activeTab === "contact" ? "block" : "hidden"}`}>
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-5 text-left">
                          <Form.Item
                            label={<span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Contact Number</span>}
                            name="contact"
                            className="text-left"
                            rules={[
                          ...
                          <Form.Item
                            label={<span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">Support Email</span>}
                            name="email"
                            className="text-left"
                            rules={[
                      ...
                      <div className={`grid grid-cols-1 md:grid-cols-2 gap-5 text-left ${activeTab === "social" ? "grid" : "hidden"}`}>
                      ...
                      <div className={`space-y-5 ${activeTab === "delivery" ? "block" : "hidden"}`}>
                          ...
                          {isRazorpayEnabled && (
                            <div className="p-4 sm:p-5 bg-white dark:bg-slate-900/40 rounded-2xl border border-gray-100 dark:border-slate-800">
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                <Form.Item label={<span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">API Key</span>} name="razorpayApiKey" className="text-left">
                                ...
                                <Form.Item label={<span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left">API Secret</span>} name="razorpayApiSecret" className="text-left">
                      ...
                      <div className={`space-y-4 ${activeTab === "secure" ? "block" : "hidden"}`}>
                          ...
                          <div className="space-y-3">
                            ...
                            <div className="rounded-2xl border border-gray-100 dark:border-slate-800 bg-gray-50/60 dark:bg-slate-900/40 p-4 overflow-visible">
                              {paymentImages.length > 0 ? (
                                ...
                                <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest">No badges selected</div>
                              )}
                            </div>
                          </div>
                        </Form.Item>
            ...
            {isFetching && <div className="text-xs text-slate-400 font-bold uppercase tracking-widest">Refreshing settings...</div>}
          </div>
        </Card>
      </Form>
      ...
      <UploadImage
        ...
        onSelect={(items: UploadItem[]) => {
          const urls = items.map((item) => item?.url || item?.path).filter(Boolean) as string[];
          if (urls.length === 0) return;
          const merged = Array.from(new Set([...paymentImages, ...urls]));
          form.setFieldsValue({ securePaymentImages: merged.join("\n") });
        }}
      />
