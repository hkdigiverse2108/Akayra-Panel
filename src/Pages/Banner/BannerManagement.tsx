                ...
                {loading && banners.length === 0 ? (
                  ...
                  banners.map((banner: any, index: number) => (
                    <tr key={banner._id} className="hover:bg-gray-50/50 dark:hover:bg-slate-800/30 transition-colors group cursor-default">
                      ...
                      <td className="px-4 sm:px-8 py-5 text-[10px] sm:text-xs font-black text-slate-900 dark:text-white hidden md:table-cell">
                        <div className="flex items-center gap-2">
                          <div className="h-7 w-7 rounded-lg bg-orange-50 dark:bg-orange-500/10 flex items-center justify-center text-orange-600 dark:text-orange-400 text-xs font-black">{banner.priority || 0}</div>
                        </div>
                      </td>
                      ...
                      <td className="px-4 sm:px-8 py-5 text-[10px] sm:text-xs font-bold text-slate-400 truncate max-w-[150px] italic underline hidden md:table-cell">{banner.pageRedirection || banner.ctaButtonRedirection || "Promotion"}</td>
                      <td className="px-4 sm:px-8 py-5 text-right">
                        <div className="flex items-center justify-end gap-1.5 sm:gap-2">
                          <Tooltip title={banner.isActive ? "Deactivate" : "Activate"}>
                            <button onClick={() => handleToggleStatus(banner)} disabled={isActionLoading} className={cn("p-2 rounded-xl transition-all shadow-sm", banner.isActive ? "text-emerald-500 bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-500/10 dark:hover:bg-emerald-500/20" : "text-slate-300 bg-slate-50 hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700")}>
                              {banner.isActive ? <ToggleRight size={20} className="sm:w-6 sm:h-6" /> : <ToggleLeft size={20} className="sm:w-6 sm:h-6" />}
                            </button>
                          </Tooltip>
                          <Tooltip title="Edit">
                            <button onClick={() => navigate(`${ROUTES.BANNERS}/edit/${banner._id}`)} className="p-2 bg-primary-50 hover:bg-primary-100 dark:bg-primary-500/10 dark:hover:bg-primary-500/20 text-primary-600 dark:text-primary-400 rounded-xl transition-all shadow-sm">
                              <Edit size={18} className="sm:w-5 sm:h-5" />
                            </button>
                          </Tooltip>
                          <Tooltip title="Delete">
                            <button onClick={() => handleDeleteClick(banner._id)} className="p-2 bg-red-50 hover:bg-red-100 dark:bg-red-500/10 dark:hover:bg-red-500/20 text-red-600 dark:text-red-400 rounded-xl transition-all shadow-sm">
                  ...
                  <div className="flex items-center justify-between gap-4 text-left">
                    <div className="text-left overflow-hidden">
                      <div className="flex items-center gap-2 mb-0.5">
                        ...
                        <div className="h-5 px-1.5 rounded-md bg-orange-50 dark:bg-orange-500/10 flex items-center justify-center text-orange-600 dark:text-orange-400 text-[8px] font-black shrink-0">PRIORITY {banner.priority || 0}</div>
                      </div>
