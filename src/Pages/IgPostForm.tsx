          ...
          <div className="space-y-4 sm:space-y-6">
            ...
            <Card className="rounded-2xl sm:rounded-3xl border border-gray-100 dark:border-slate-800 bg-white dark:bg-slate-900 p-4 sm:p-6 text-left space-y-4">
              <Form.Item label={<span className="font-bold text-slate-600 dark:text-slate-400 text-left">Type</span>} name="type" rules={[{ required: true, message: 'Type is required' }]} className="text-left mb-0">
                <Select size="large" placeholder="Select type" options={[ { label: 'Image', value: 'image' }, { label: 'Video', value: 'video' }, ]} className="custom-select h-12 rounded-xl overflow-hidden [&_.ant-select-selector]:!rounded-xl text-left dark:bg-slate-800 dark:text-white dark:border-slate-700" />
              </Form.Item> <br />

              {typeValue === 'video' ? (
                <Form.Item  label={<span className="font-bold text-slate-600 dark:text-slate-400 text-left">Video URL</span>}  name="video"  rules={[({ getFieldValue }) => ({ validator(_, value) { return getFieldValue('type') === 'video' && !value ? Promise.reject(new Error('Video URL is required')) : Promise.resolve(); } }) ]} className="text-left mb-0" >
                ...
                <Form.Item label={<span className="font-bold text-slate-600 dark:text-slate-400 text-left">Image URL</span>} name="image" rules={[ ({ getFieldValue }) => ({ validator(_, value) { return getFieldValue('type') === 'image' && !value ? Promise.reject(new Error('Image URL is required')) : Promise.resolve(); } }) ]} className="text-left mb-0" >
                  <Input size="large" placeholder="https://..." prefix={<ImageIcon size={14} className="text-slate-400" />}
                     className="h-12 rounded-xl focus:ring-primary-500 text-left dark:bg-slate-800 dark:text-white dark:border-slate-700"/>
                </Form.Item>
              ) : (
                <div className="text-xs font-bold text-slate-400 dark:text-slate-500 text-left">
                  Select a type to enable the asset input.
                </div>
              )}
            </Card>
          </div>
