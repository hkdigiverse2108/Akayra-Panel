...
import { Layers, ImageIcon, Save, ArrowLeft, X } from "lucide-react";
    ...
    mutation.mutate(payload, {
      onSuccess: (res: any) => {
        if (res.status === 200 || res.status === 201) {
          ...
          queryClient.refetchQueries({ queryKey: [KEYS.CATEGORY.ALL] });
          queryClient.refetchQueries({ queryKey: [KEYS.CATEGORY.BASE] });
          queryClient.refetchQueries({ queryKey: [KEYS.CATEGORY.DROPDOWN] });
    ...
    <div className="max-w-5xl mx-auto space-y-4 sm:space-y-8 animate-in slide-in-from-bottom-4 duration-500 px-4 sm:px-0 pb-12 sm:pb-0 text-left">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 sticky top-0 z-20 bg-gray-50/90 dark:bg-slate-950/90 backdrop-blur-md py-4 -mx-4 px-4 sm:mx-0 sm:px-0 border-b border-gray-100 dark:border-slate-800 text-left">
        <div className="flex items-center gap-3 sm:gap-4 text-left">
          <button onClick={() => navigate(ROUTES.CATEGORIES)} className="p-2 sm:p-3 hover:bg-white dark:hover:bg-slate-800 rounded-xl sm:rounded-2xl transition-all shadow-sm hover:shadow text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 border border-gray-100 dark:border-slate-800">
          ...
          <div className="text-left overflow-hidden">
            <Breadcrumb
              ...
              items={[
                ...
                {
                  title: (
                    <span className="cursor-pointer hover:text-primary-500 transition-colors" onClick={() => navigate(ROUTES.CATEGORIES)}>
                      Categories
                    </span>
                  ),
                },
            ...
            />
            <h1 className="text-xl sm:text-3xl font-black text-slate-900 dark:text-white tracking-tight text-left truncate">{isEditMode ? "Edit Category" : "New Category"}</h1>
          </div>
        </div>
        <div className="flex items-center gap-2 sm:gap-3">
          <Button variant="ghost" onClick={() => navigate(ROUTES.CATEGORIES)} className="h-10 sm:h-12 px-4 sm:px-6 rounded-xl sm:rounded-2xl border-2 font-bold text-slate-600 dark:text-slate-400 flex-1 sm:flex-none">
            Cancel
          </Button>
        ...
        <div className="text-left">
          <Form form={form} layout="vertical" onFinish={onFinish} initialValues={{ isActive: true }} requiredMark={false} className="max-w-4xl mx-auto py-2 sm:py-4 text-left">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 text-left">
              ...
              <Form.Item label={<span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left text-left">Category Name</span>} name="name" rules={[{ required: true, message: "Please enter category name" }]} className="col-span-2 md:col-span-1 text-left">
                <Input size="large" placeholder="e.g. Mens Fashion" className="h-12 rounded-xl bg-gray-50/50 dark:bg-slate-800 border-0 px-5 font-bold focus:ring-primary-500 text-left" />
              </Form.Item>

              <Form.Item label={<span className="text-[10px] font-black text-slate-400 uppercase tracking-widest text-left text-left">Image URL</span>} name="image" className="col-span-2 md:col-span-1 text-left">
                  ...
                  {watchedImage ? (
                    <div className="flex items-center justify-between gap-3 px-3 py-2 rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900">
                      <div className="flex items-center gap-3 min-w-0">
                        <img src={watchedImage} alt="Category" className="h-12 w-12 rounded-xl object-cover border border-slate-200 dark:border-slate-700" />
