...
const MainLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-slate-950 transition-colors duration-300">
      ...
      {isSidebarOpen && layout === "vertical" && <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-40 lg:hidden transition-all duration-300 animate-in fade-in" onClick={() => setSidebarOpen(false)} />}

      {/* Mobile Sidebar overlay for horizontal layout */}
      {isSidebarOpen && layout === "horizontal" && (
        <div className="lg:hidden">
          <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-40 transition-all duration-300 animate-in fade-in" onClick={() => setSidebarOpen(false)} />
          <Sidebar />
        </div>
      )}
      ...
      <div className={cn("flex flex-col min-h-screen transition-all duration-300", getSidebarWidth())}>
        ...
        <footer className="py-6 px-8 border-t border-gray-100 dark:border-slate-800 text-center">
          <p className="text-sm text-gray-500 dark:text-slate-400 font-medium">&copy; {new Date().getFullYear()} Akayra Admin Panel. All rights reserved.</p>
        </footer>
      </div>
    </div>
  );
