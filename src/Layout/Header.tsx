...
const Header: React.FC = () => {
  ...
  return (
    <header className="h-16 bg-white dark:bg-slate-900 border-b border-gray-200 dark:border-slate-800 flex items-center justify-between px-6 sticky top-0 z-40 transition-colors">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-500 dark:text-slate-400 mt-1">
                      {profileUser?.email || 'admin@akayra.com'}
                    </p>
                  </div>
                </div>
                <div className="mt-3 flex items-center gap-2">
                  <span className="rounded-full bg-primary-500/10 text-primary-600 dark:text-primary-300 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide">
                    {profileUser?.role || 'Administrator'}
                  </span>
                  <span className="rounded-full bg-emerald-500/10 text-emerald-600 dark:text-emerald-300 px-2.5 py-1 text-[11px] font-semibold uppercase tracking-wide">
                    Active
                  </span>
                </div>
              </div>
      ...
      <ConfirmModal isOpen={isLogoutOpen} onClose={() => setIsLogoutOpen(false)} onConfirm={() => {   setIsLogoutOpen(false);   setIsProfileOpen(false);   logout(); }}
